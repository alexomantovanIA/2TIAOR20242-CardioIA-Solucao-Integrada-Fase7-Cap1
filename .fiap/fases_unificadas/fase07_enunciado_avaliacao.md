 ...
 Após percorrer as etapas de coleta de dados (Fase 1), diagnóstico automatizado (Fase 2), monitoramento IoT (Fase 3), análise de imagens (Fase 4), assistência conversacional (Fase 5) e orquestração multiagente (Fase 6), a Fase 7 tem por objetivo consolidar a solução em um produto integrado e usável.

 Esta fase enfatiza a integração de componentes (back-end, front-end, serviços em nuvem, LLMs e sensores) e o deploy profissional das interfaces Web e Mobile, além da evolução da lógica de sensores para MicroPython em dispositivos mais potentes (por exemplo, Raspberry Pi). O foco é entregar um MVP funcional, seguro e centrado no usuário.

 ## Objetivo geral

 Desenvolver o MVP final e funcional da plataforma CardioIA, capaz de:

 - Publicar a aplicação Web com CI/CD ativo.
 - Gerar um arquivo `.apk` via Expo EAS Build para a aplicação Mobile.
 - Refinar a experiência do usuário (UX) e o fluxo de apresentação dos indicadores de risco.
 - Integrar os módulos técnicos das fases anteriores em um backend integrador em Python.
 - Simular/portar a lógica de captura de sensores para MicroPython.
 - Trabalhar em equipe (recomendado grupos de 4 a 5 integrantes; 1 ponto extra para equipes organizadas conforme recomendado).
 ...
 - **Web**: publicar a aplicação na Vercel (ou similar), configurando o `vercel.json` para suporte a rotas SPA e habilitar deploys automáticos a cada push no GitHub (CI/CD).
 - **Mobile**: configurar `app.json` e `eas.json` para realizar builds na nuvem do Expo, gerando um `.apk` funcional.
 - **Validação**: testar a aplicação Web e instalar o `.apk` em um dispositivo real, validando fluxo de login e visualização dos dados cardíacos.

 ### Entregáveis (Parte 1)

 - Repositório GitHub privado (compartilhado com o tutor) com o código-fonte Web (React + Vite) e Mobile (React Native + Expo).
 - Arquivos obrigatórios: `vercel.json`, `app.json` (com `android.package` em formato de domínio invertido) e `eas.json` com perfil `preview` para geração de APK.
 - `README.md` com a URL pública do deploy Web, link/QR do build do `.apk` no dashboard do Expo, prints do deploy e instruções de instalação.

 ### PARTE 2 – Integração do Ecossistema e Arquitetura Final

 Desenvolver o núcleo integrador em Python garantindo:

 - **Back-end Integrador**: serviço Python que conecte as interfaces aos motores de IA (modelos preditivos da Fase 6 e LLMs da Fase 5).
 - **Transição para MicroPython**: converter a lógica de captura/processing de sensores (temperatura, batimentos simulados) para MicroPython e simular execução em ESP32/Raspberry Pi no Wokwi.
 - **Arquitetura Final**: consolidar o fluxo Sensor → MicroPython → Backend Python → APIs de IA → UI.

 ### Entregáveis (Parte 2)

 - Código-fonte integrado no GitHub (pasta `backend` com scripts Python e pasta `iot` com script `.py` para MicroPython).
 - Link público para projeto Wokwi demonstrando a simulação MicroPython com análise de sinais e feedback visual (LED/OLED).

 ## Relatório Técnico

 - Formato: PDF, máximo de cinco páginas.
 - Conteúdo obrigatório: diagrama de arquitetura final, descrição do fluxo fim-a-fim e link para o vídeo demonstrativo (até 5 minutos) mostrando a integração completa.

 ## Critérios de Avaliação (10 pontos totais)

 | Critério | Pontos |
 | :--- | :---: |
 | URLs funcionais (deploy Web) e build mobile (.apk via EAS) acessíveis | 3.0 |
 | Unificação funcional do backend com interfaces e motores de IA | 2.5 |
 ...
 | Grupo organizado de 4 a 5 integrantes (Extra) | 1.0 (Extra) |

 ## IR ALÉM 1 – Mineração de Processos e Conformidade Clínica

 Expandir o sistema para analisar a eficiência do fluxo assistencial (AI for RPA). A proposta inclui:

 - Análise de event logs do atendimento de Infarto Agudo do Miocárdio (IAM).
 - Descoberta de variantes de processo (ex.: casos que pularam ECG).
 - Cálculo de tempos de ciclo e identificação de gargalos.
 - Verificação de conformidade entre fluxo real e protocolo clínico esperado.

 Entregáveis: notebook Python (pm4py), relatório curto (máx. 2 páginas) e evidências de execução (prints).

 ## IR ALÉM 2 – Recuperação de Casos com Embeddings Visuais

 Implementar pipeline de CBIR para radiografias de tórax:

 - Extração de embeddings visuais com modelo pré-treinado.
 - Indexação dos embeddings em espaço vetorial.
 - Recuperação de exames semelhantes e avaliação com Precision@K.

 Entregáveis: notebook no Colab com pipeline, relatório técnico (máx. 2 páginas) e análise qualitativa dos vizinhos recuperados.

 ## Mensagem final

 Na Fase 7, o CardioIA atinge sua maturidade máxima, consolidando-se como uma plataforma integrada onde o processamento em borda (MicroPython), a orquestração em Python e o deploy profissional entregam inteligência útil a médicos e pacientes.

 ---

 > Observação: formatação ajustada para seguir o padrão das fases anteriores. Preserve o conteúdo técnico; atualize links/nomes de arquivos conforme necessário no repositório.
...
URLs funcionais (Vercel) e build mobile (.apk via EAS) acessíveis.

3.0

Unificação funcional do backend com as interfaces e motores de IA.

2.5

Conversão e funcionalidade correta da lógica de sensores no Wokwi.

1.5

Clareza no diagrama final e fluidez na comunicação dos dados.

1.5

Qualidade do README, do relatório PDF e clareza da demonstração.

1.5

Grupo organizado de 4 a 5 integrantes (Extra).
...
Com base nos conceitos da disciplina de AI for Robotic Process Automation (AIRPA), expandir o sistema analisando a eficiência do fluxo assistencial. A proposta é implementar um mecanismo que:

Analise o event log do atendimento de Infarto Agudo do Miocárdio (IAM).
Identifique as variantes de processo (exemplo: casos que pularam o ECG ou inverteram etapas).
Calcule o tempo de ciclo e identifique gargalos críticos entre as atividades.
Realize uma verificação de conformidade comparando o fluxo real com o protocolo clínico esperado.
  

Entregável:

Notebook Python implementando a descoberta de processos (via pm4py).
Relatório técnico (máximo de duas páginas) explicando as variantes encontradas e os impactos clínicos dos atrasos detectados.
Evidência de execução (print das variantes e do gráfico de performance).
  

IR ALÉM 2 – RECUPERAÇÃO DE CASOS COM EMBEDDINGS VISUAIS

Com base nos conceitos de Visão Computacional (VC), expandir o sistema para suporte à revisão de casos através de busca por similaridade. A proposta é estruturar um pipeline de Content-Based Image Retrieval (CBIR) que:

Extraia embeddings visuais de radiografias de tórax utilizando um modelo pré-treinado.
Indexe esses embeddings em um espaço vetorial.
Recupere os exames semanticamente mais semelhantes a uma imagem de consulta.
Avalie a precisão da recuperação através da métrica Precision@K.
 

Entregável:

Notebook no Google Colab com o pipeline de extração, indexação e busca.
Relatório técnico (máximo de duas páginas) apresentando os resultados visuais da busca.
Análise qualitativa dos vizinhos recuperados e discussão sobre as limitações do modelo.
